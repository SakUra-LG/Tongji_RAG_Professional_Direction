...
## 1. 概述与变更说明
当前前端只使用 `auto` 智能问答入口，不允许用户手动选择模块。后端根据身份自动
限制检索范围：访客仅公开数据，学生和教师自动连接公开、校内爬取和本人个人数据。

### 1.1 业务模块枚举 (`type`)
前端在调用 Session 和 Chat 接口时固定使用 `auto`：
| 模块类型 (`type`) | 描述 | 权限要求 |
| :--- | :--- | :--- |
| `auto` | 个人数据优先、已有数据库其次、通用大模型最后 | 所有用户 |
| `public` | 公开问答 (校务公开/FAQ) | 所有用户 (含游客) |
| `academic` | 学术科研 (文献/知识库) | 学生、教师、访问学者 |
| `internal` | 内部行政 (通知/办事) | 学生、教师 |
| `personal` | 个人档案 (画像/推荐) | 学生、教师 |

后四类接口仅为旧客户端兼容保留，当前前端不展示对应选择器。
...
* **Method**: `POST`
* **Body**: `{}` (空)
* **Response**: 包含临时 Token，用于后续 `auto` 调用；访客仍只能检索公开数据。
